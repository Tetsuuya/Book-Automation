...
def add_multimedia_box(doc, mod_title, vid_title, channel, link, qr_code_text, timestamps, questions):
    ...
    r_qr = p_qr.add_run(f"[📱 {qr_code_text}]  Scan to Watch on YouTube Mobile or Smart TV")
    r_qr.font.name = 'Arial'
    r_qr.font.size = Pt(9.5)
...
def add_callout_box(doc, title, body_lines, color_fill="F4F7FA", border_color="1B365D"):
    t = doc.add_table(rows=1, cols=1)
    t.alignment = WD_TABLE_ALIGNMENT.CENTER
    c = t.rows[0].cells[0]
